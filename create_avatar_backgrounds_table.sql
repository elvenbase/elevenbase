...
-- Create function to create table (for RPC call)
CREATE OR REPLACE FUNCTION create_avatar_backgrounds_table()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  ...
  DROP POLICY IF EXISTS "Users can update their own avatar backgrounds" ON avatar_backgrounds;
  CREATE POLICY "Users can update their own avatar backgrounds" ON avatar_backgrounds
    FOR UPDATE USING (auth.uid() = created_by);

  DROP POLICY IF EXISTS "Users can delete their own avatar backgrounds" ON avatar_backgrounds;
  CREATE POLICY "Users can delete their own avatar backgrounds" ON avatar_backgrounds
    FOR DELETE USING (auth.uid() = created_by);
END;
$$;
