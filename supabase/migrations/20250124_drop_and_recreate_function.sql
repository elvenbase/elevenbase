-- DROP AND RECREATE FUNCTION WITH OWNER ALWAYS ACTIVE
-- Date: 2025-01-24

-- 1. Drop la funzione esistente
DROP FUNCTION IF EXISTS public.create_team_for_new_user(text,text,text,text,text,uuid,text);

-- 2. Ricrea la funzione con owner SEMPRE active
...
-- 3. FIX IMMEDIATO: Aggiorna TUTTI gli owner esistenti a status active
...
-- 4. Verifica che tutti gli owner siano attivi
SELECT 
    u.email,
    t.name as team_name,
    tm.role,
    tm.status,
    CASE 
        WHEN t.owner_id = u.id THEN '✅ OWNER - Ora è ACTIVE'
        ELSE 'Member'
    END as note
FROM auth.users u
