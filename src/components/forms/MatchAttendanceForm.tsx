...
import { CheckCircle, XCircle, Users, Check, Clock, UserPlus } from 'lucide-react';
import { PlayerAvatar } from '@/components/ui/PlayerAvatar';
import { toast } from 'sonner';
import { usePlayers, useMatchAttendance, useUpsertMatchAttendance, useBulkUpdateMatchAttendance, useMatchTrialistInvites, useUpdateMatchTrialistInvite } from '@/hooks/useSupabaseData';
...
const MatchAttendanceForm = ({ matchId }: MatchAttendanceFormProps) => {
  ...
  const updateTrialistInvite = useUpdateMatchTrialistInvite();
  ...
  const handleSelfStatusChange = async (playerId: string, status: string) => {
    try {
      ...
      await upsert.mutateAsync({ match_id: matchId, player_id: playerId, status, self_registered: !!exists?.self_registered });
      toast.success('Auto-registrazione aggiornata');
      refetch();
    } catch (error: any) {
      toast.error('Errore aggiornando auto-registrazione: ' + error.message);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleCoachConfirmationChange = async (playerId: string, confirmationStatus: string) => {
    try {
      ...
      toast.error('Errore aggiornando conferma: ' + error.message);
    } finally {
      setIsLoading(false);
    }
  };
              ...
              <div key={player.id}>
                ...
                <div className="hidden md:grid md:grid-cols-12 gap-4 p-4 items-center hover:bg-muted/30">
                  ...
                  <div className="col-span-2 text-center">
                    <Select value={rec?.status || 'pending'} onValueChange={(value) => handleSelfStatusChange(player.id, value)}>
                  ...
                  <div className="col-span-2">
                    <Select value={rec?.coach_confirmation_status || 'pending'} onValueChange={(value) => handleCoachConfirmationChange(player.id, value)}>
                      <SelectTrigger className="h-8">
                        <SelectValue placeholder="Seleziona" />
                      </SelectTrigger>
                      <SelectContent>
        ...
        <div className="divide-y">
          ...
          {trialistInvites.map((t: any) => (
            <div key={t.trialist_id} className="p-4 flex items-center justify-between gap-4">
              ...
              <div className="w-48">
                <Select value={t.status || 'pending'} onValueChange={(value) => updateTrialistInvite.mutate({ match_id: matchId, trialist_id: t.trialist_id, status: value as any })}>
