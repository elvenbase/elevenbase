...
const EditPlayerForm = ({ player, triggerAs = 'button', triggerLabel = 'Modifica', triggerClassName = '', children }: EditPlayerFormProps) => {
  ...
  const handleAvatarUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      try { localStorage.setItem('playerAvatarUpdatedAt', `${player.id}:${Date.now()}`) } catch (e) { void e }
        ...
        <DialogHeader>
          ...
          <DialogClose asChild>
            <Button 
              variant="outline" 
              size="sm" 
              className="absolute right-4 top-4 h-8 w-16 bg-background border-border shadow-sm hover:bg-muted"
