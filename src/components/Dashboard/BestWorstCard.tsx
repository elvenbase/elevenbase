...
type Props = {
  title: string
  metricLabel: string
  best?: { player: PlayerRef; value: number } | null
  worst?: { player: PlayerRef; value: number } | null
}

export const BestWorstCard = ({ title, metricLabel, best, worst }: Props) => {
  const Item = ({ kind, data }: { kind: 'best'|'worst'; data?: { player: PlayerRef; value: number } | null }) => {
    if (!data) return (
        ...
        <div className="flex items-center justify-between p-3 rounded-lg bg-muted/40 hover:bg-muted transition-smooth">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-muted-foreground">{metricLabel}: <span className="text-foreground font-semibold">{data.value}</span></div>
            </div>
          </div>
          {kind === 'best' ? (
