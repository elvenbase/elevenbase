...
export const TopLeaderCard = ({ metricLabel, valueUnit, variant = 'neutral', item, distribution = [], onSegmentOpen }: Props) => {
  ...
  return (
    <div className="group [perspective:1000px] min-h-[220px] sm:min-h-[200px] lg:min-h-[224px] w-full">
        ...
        <Link to={to} className="block absolute inset-0 [backface-visibility:hidden]">
          <Card className={`p-4 sm:p-[18px] lg:p-[22px] bg-card/80 border border-border rounded-2xl shadow-card hover:shadow-glow transition-smooth h-full overflow-hidden`} onClick={(e)=>{ e.preventDefault(); setFlipped(true) }}>
            <div className="flex flex-col h-full animate-in fade-in-0">
              ...
              {/* Hero band */}
              <div className="relative flex-1 flex sm:flex-row flex-col items-start gap-3 sm:gap-4 px-1 sm:px-2 py-2">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0 max-w-full">
                      <div className="line-clamp-2 break-words font-semibold text-[20px] sm:text-[20px] lg:text-[22px]">{p.first_name} {p.last_name}</div>
                      <div className="mt-1 text-[13px] text-muted-foreground">Migliore {metricLabel.toLowerCase()} nel periodo</div>
                    </div>
                    {/* Right pill only on >=sm */}
                  ...
                  {/* Mobile pill under name */}
                  <div className="mt-2 sm:hidden flex items-center justify-end">
                    <div className="inline-flex items-center gap-1.5 rounded-full h-9 px-4" style={{ backgroundColor: metricStyle.headerBg }}>
                      <SectionIcon className="h-5 w-5" style={{ color: metricStyle.accent }} aria-hidden />
              ...
              {/* Footer band */}
              <div className="flex items-center justify-between h-7 text-[12px] sm:text-[13px]" style={{ color: '#5A5A5A' }}>
